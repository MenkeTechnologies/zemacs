...
impl Uri {
    // This clippy allow mirrors zemacs_stdx::Url::from_file_path
    #[allow(clippy::result_unit_err)]
    pub fn to_url(&self) -> Result<zemacs_stdx::Url, ()> {
        match self {
            Uri::File(path) => zemacs_stdx::Url::from_file_path(path),
        }
    }
...
fn convert_url_to_uri(url: &zemacs_stdx::Url) -> Result<Uri, UrlConversionErrorKind> {
    if url.scheme() == "file" {
        url.to_file_path()
            .map(|path| Uri::File(zemacs_stdx::path::normalize(path).into()))
...
impl TryFrom<zemacs_stdx::Url> for Uri {
    type Error = UrlConversionError;

    fn try_from(url: zemacs_stdx::Url) -> Result<Self, Self::Error> {
