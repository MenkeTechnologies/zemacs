...
pub use zemacs_core::diagnostic::LanguageServerId;
...
pub enum Error {
    ...
    Unhandled,
    #[error(transparent)]
    ExecutableNotFound(#[from] zemacs_stdx::env::ExecutableNotFoundError),
    #[error(transparent)]
...
pub mod util {
    use super::*;
    use zemacs_core::line_ending::{line_end_byte_index, line_end_char_index};
    use zemacs_core::snippets::{RenderedSnippet, Snippet, SnippetRenderCtx};
    use zemacs_core::{chars, RopeSlice};
    use zemacs_core::{diagnostic::NumberOrString, Range, Rope, Selection, Tendril, Transaction};
    ...
    pub fn diagnostic_to_lsp_diagnostic(
        doc: &Rope,
        diag: &zemacs_core::diagnostic::Diagnostic,
    ...
    pub fn lsp_pos_to_pos(
        ...
        //
        // FIXME: Zemacs does not fully comply with the LSP spec for line terminators.
        // The LSP standard requires that line terminators are ['\n', '\r\n', '\r'].
        // Without the unicode-linebreak feature disabled, the `\r` terminator is not handled by zemacs.
        // With the unicode-linebreak feature, zemacs recognizes multiple extra line break chars
    ...
    pub fn generate_transaction_from_edits(
        ...
        if edits.len() == 1 {
            ...
            if is_document_replacement {
                ...
                return zemacs_core::diff::compare_ropes(doc, &new_text);
            }
        }
        ...
        let mut last_end = 0;
        let mut changes: Vec<(usize, usize, Option<zemacs_core::Tendril>)> =
...
pub struct Registry {
    ...
    syn_loader: Arc<ArcSwap<zemacs_core::syntax::Loader>>,
    pub incoming: SelectAll<UnboundedReceiverStream<(LanguageServerId, Call)>>,
    pub file_event_handler: file_event::Handler,
}

impl Registry {
    pub fn new(syn_loader: Arc<ArcSwap<zemacs_core::syntax::Loader>>) -> Self {
        Self {
    ...
    /// See zemacs_view::editor::Editor::refresh_language_servers
...
fn start_client(
    ...
    let (workspace, workspace_is_cwd) = zemacs_loader::find_workspace();
...
pub fn find_lsp_workspace(
    ...
    let mut file = if file.is_absolute() {
        ...
        let current_dir = zemacs_stdx::env::current_working_dir();
