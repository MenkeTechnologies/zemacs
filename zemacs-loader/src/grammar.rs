...
fn ensure_git_is_available() -> Result<()> {
    zemacs_stdx::env::which("git")?;
    Ok(())
}

/// Print a notice if the current workspace has a `.zemacs/languages.toml` that we *would* have
...
fn warn_if_workspace_languages_skipped(trust: &crate::workspace_trust::WorkspaceTrust) {
    ...
    println!(
        "Note: workspace `{}` was skipped because the workspace is not trusted. Run \
         `:workspace-trust` from an interactive zemacs session in this workspace to opt in.",
...
fn get_grammar_configs() -> Result<Vec<GrammarConfiguration>> {
    ...
    // them into `.so` files zemacs later loads at runtime. If we let workspace
    // `.zemacs/languages.toml` in through `fully_trusted`, a malicious workspace could inject a
