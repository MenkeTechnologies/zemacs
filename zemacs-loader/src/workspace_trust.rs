//! Workspace trust.
//!
//! Zemacs can load workspace-local configuration (`.zemacs/`) and launch language servers, both of
...
//! A grant snapshots a hash of every file under `.zemacs/`. If those files change later the
...
//! multiple concurrent zemacs instances writing trust for *different* workspaces (different
...
//! bypasses the `.zemacs/` hash pin (changes to local config are never re-checked) and it trusts any
...
pub enum TrustQuery {
    ...
    Dap,
    /// Query whether `.zemacs/` config can be loaded
    LocalConfig,
...
pub enum TrustStatus {
    ...
    Untrusted,
    /// Workspace was previously trusted, but the `.zemacs/` tree has changed since the grant — the
...
pub enum ImplicitTrustLevel {
    ...
    None,
    /// Zemacs-launched server processes (LSP and DAP) start implicitly. Workspace-local config and
...
pub fn build_trusted_globs(patterns: &[String]) -> GlobSet {
    ...
    for pattern in patterns {
        let expanded = zemacs_stdx::path::expand(pattern);
...
struct CacheEntry {
    status: TrustStatus,
    /// Whether `.zemacs/config.toml` or `.zemacs/languages.toml` exists in the workspace at the time
...
impl WorkspaceTrust {
    ...
    /// disk. This catches external mutations to `.zemacs/` that happened while zemacs was running
    ...
    /// `.zemacs/` changed) from *Untrusted* (never trusted)
    ...
    /// Mark `workspace` trusted. Snapshots the current `.zemacs/` hash.
...
fn has_local_config(workspace: &Path) -> bool {
    workspace.join(".zemacs").join("config.toml").exists()
        || workspace.join(".zemacs").join("languages.toml").exists()
}

fn demote_for_query(status: TrustStatus, query: TrustQuery) -> TrustStatus {
    // Stale workspaces have their `.zemacs/` config changed since trust was granted. LSP launches
...
// filename derives from the path, so concurrent zemacs instances writing trust for *different*
...
/// SHA-256 of all files under `.zemacs/`, used to detect changes to local config after trust was
/// granted. Returns `None` if `.zemacs/` is absent or has no files, so a workspace with no local
/// config can still be trusted.
pub fn compute_workspace_hash(workspace: &Path) -> Option<String> {
    let zemacs_dir = workspace.join(".zemacs");
    if !zemacs_dir.is_dir() {
        return None;
    }

    let mut files: Vec<PathBuf> = Vec::new();
    walk(&zemacs_dir, &mut files);
    if files.is_empty() {
        return None;
    }
    files.sort();
    ...
    let mut hasher = Sha256::new();
    for file in &files {
        let rel = file.strip_prefix(&zemacs_dir).unwrap_or(file);
...
fn walk(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if direct.is_dir() {
            ...
            // Symlinks to files (extremely common: dotfiles managers symlink `.zemacs/config.toml`
...
mod test {
    ...
    #[test]
    fn hash_changes_when_zemacs_dir_changes() {
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path();

        let empty = compute_workspace_hash(workspace);
        assert_eq!(empty, None, ".zemacs/ absent should hash to None");
        ...
        write_file(&workspace.join(".zemacs").join("languages.toml"), "");
    ...
    fn set_config_invalidates_cache_so_stale_is_detected() {
        ...
        // while zemacs was running, subsequent queries kept returning Trusted even though
        ...
        // External mutation while zemacs is running.
        write_file(&workspace.join(".zemacs").join("config.toml"), "a = 2");
    ...
    fn hash_includes_symlinked_config_file() {
        // Regression: `.zemacs/config.toml` is commonly a symlink to an external location (dotfiles
        ...
        fs::create_dir_all(workspace.join(".zemacs")).unwrap();
        symlink(&external, workspace.join(".zemacs").join("config.toml")).unwrap();
    ...
    fn hash_distinguishes_file_split() {
        ...
        let split = dir1.path();
        write_file(&split.join(".zemacs").join("foo.toml"), "a");
        write_file(&split.join(".zemacs").join("bar.toml"), "b");

        let dir2 = tempfile::tempdir().unwrap();
        let merged = dir2.path();
        write_file(&merged.join(".zemacs").join("foo.toml"), "a\0bar.toml\0b");
