...
use zemacs_stdx::{env::current_working_dir, path};
...
/// 3. `ZEMACS_RUNTIME` (if environment variable is set)
/// 4. `ZEMACS_DEFAULT_RUNTIME` (if environment variable is set *at build time*)
/// 5. subdirectory of path to zemacs executable (always included)
///
...
fn prioritize_runtime_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("ZEMACS_RUNTIME") {
    ...
    if let Some(dir) = std::option_env!("ZEMACS_DEFAULT_RUNTIME") {
...
pub fn workspace_config_file() -> PathBuf {
    find_workspace().0.join(".zemacs").join("config.toml")
}

pub fn workspace_lang_config_file() -> PathBuf {
    find_workspace().0.join(".zemacs").join("languages.toml")
}
...
/// and returns the first directory that contains either `.git`, `.svn`, `.jj` or `.zemacs`.
