...
/// A wrapper around `lsp::Location` that swaps out the LSP URI for `zemacs_core::Uri` and adds
...
pub(crate) fn code_actions_for_range(
    doc: &Document,
    range: zemacs_core::Range,
    only: Option<Vec<CodeActionKind>>,
    trigger_kind: CodeActionTriggerKind,
) -> Vec<(
    impl Future<Output = Result<Option<Vec<CodeActionOrCommand>>, zemacs_lsp::Error>>,
    LanguageServerId,
)> {
    let mut seen_language_servers = HashSet::new();
        ...
        .filter_map(|language_server| {
            ...
            let code_action_context = lsp::CodeActionContext {
                diagnostics: doc
                    ...
                    .iter()
                    .filter(|&diag| {
                        range.overlaps(&zemacs_core::Range::new(diag.range.start, diag.range.end))
                    })
...
fn code_action_on_save_step(
    ...
    let build_request = move |editor: &mut Editor| -> Option<Job> {
        ...
        let full_range = zemacs_core::Range::new(0, doc.text().len_chars());
...
/// several edits.
type ResolveFuture =
    std::pin::Pin<Box<dyn Future<Output = Result<lsp::CodeAction, zemacs_lsp::Error>> + Send>>;
enum ResolveStep {
...
fn goto_impl(editor: &mut Editor, compositor: &mut Compositor, locations: Vec<Location>) {
    let cwdir = zemacs_stdx::env::current_working_dir();
...
where
    ...
    F: Future<Output = zemacs_lsp::Result<Option<lsp::GotoDefinitionResponse>>> + 'static + Send,
{
...
pub fn rename_symbol(cx: &mut Context) {
    fn get_prefill_from_word_boundary(editor: &Editor) -> String {
        ...
        if primary_selection.len() > 1 {
            ...
            use zemacs_core::textobject::{textobject_word, TextObject};
...
fn compute_inlay_hints_for_view(
    ...
    let range = zemacs_lsp::util::range_to_lsp_range(
        doc_text,
        zemacs_core::Range::new(first_char_in_range, last_char_in_range),
