...
use zemacs_core::Uri;
use zemacs_event::{cancelable_future, register_hook, send_blocking};
use zemacs_lsp::{lsp, LanguageServerId};
use zemacs_view::document::Mode;
use zemacs_view::events::{
    DiagnosticsDidChange, DocumentDidChange, DocumentDidOpen, LanguageServerInitialized,
};
use zemacs_view::handlers::diagnostics::DiagnosticEvent;
use zemacs_view::handlers::lsp::{PullAllDocumentsDiagnosticsEvent, PullDiagnosticsEvent};
...
impl zemacs_event::AsyncHook for PullDiagnosticsHandler {
...
impl zemacs_event::AsyncHook for PullAllDocumentsDiagnosticHandler {
        ...
        loop {
            match cancelable_future(futures.next(), &cancel).await {
                ...
                Some(Some((Err(err), DiagnosticProvider::Lsp { server_id, .. }, _))) => {
                    let parsed_cancellation_data = if let zemacs_lsp::Error::Rpc(error) = err {
