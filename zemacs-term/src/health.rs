...
pub fn general() -> std::io::Result<()> {
    ...
    let config_file = zemacs_loader::config_file();
    let lang_file = zemacs_loader::lang_config_file();
    let log_file = zemacs_loader::log_file();
    let rt_dirs = zemacs_loader::runtime_dirs();
...
pub fn languages_selection() -> std::io::Result<()> {
    let selection = zemacs_loader::grammar::get_grammar_names().unwrap_or_default();
    languages(selection)
}
...
fn languages(selection: Option<HashSet<String>>) -> std::io::Result<()> {
    ...
    let trust = zemacs_loader::workspace_trust::WorkspaceTrust::fully_trusted();
    ...
    let check_binary_with_name = |cmd: Option<(&str, &str)>| match cmd {
        Some((name, cmd)) => match zemacs_stdx::env::which(cmd) {
...
fn probe_parser(grammar_name: &str) -> std::io::Result<()> {
    ...
    match zemacs_loader::grammar::get_language(grammar_name) {
...
fn probe_protocols<'a, I: Iterator<Item = (&'a str, &'a str)> + 'a>(
    ...
    for (name, cmd) in server_cmds {
        let (diag, icon) = match zemacs_stdx::env::which(cmd) {
