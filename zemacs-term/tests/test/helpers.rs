...
impl LineFeedHandling {
    ...
    pub fn apply(&self, text: &str) -> String {
        let line_end = match self {
            LineFeedHandling::Native => zemacs_core::NATIVE_LINE_ENDING,
...
pub async fn test_key_sequence_with_input_text<T: Into<TestCase>>(
    ...
    let (view, doc) = zemacs_view::current!(app.editor);
...
pub fn test_syntax_loader(overrides: Option<String>) -> zemacs_core::syntax::Loader {
    let mut lang = zemacs_loader::config::default_lang_config();

    if let Some(overrides) = overrides {
        let override_toml = toml::from_str(&overrides).unwrap();
        lang = zemacs_loader::merge_toml_values(lang, override_toml, 3);
    }

    zemacs_core::syntax::Loader::new(lang.try_into().unwrap()).unwrap()
}
...
pub fn test_editor_config() -> zemacs_view::editor::Config {
    zemacs_view::editor::Config {
        lsp: LspConfig {
            enable: false,
            ..Default::default()
        },
...
pub struct AppBuilder {
    args: Args,
    config: Config,
    syn_loader: zemacs_core::syntax::Loader,
...
impl AppBuilder {
    ...
    pub fn with_file<P: Into<PathBuf>>(
        mut self,
        path: P,
        pos: Option<zemacs_core::Position>,
    ) -> Self {
        self.args
            .files
    ...
    pub fn with_config(mut self, mut config: Config) -> Self {
        let keys = replace(&mut config.keys, zemacs_term::keymap::default());
    ...
    pub fn with_lang_loader(mut self, syn_loader: zemacs_core::syntax::Loader) -> Self {
