...
use zemacs_core::command_line::{ExpansionKind, Token, TokenKind, Tokenizer};
...
pub enum Variable {
    ...
    /// Nearest ancestor directory of the current working directory that contains `.git`, `.svn`, `jj` or `.zemacs`
    WorkspaceDirectory,
...
fn expand_variable(editor: &Editor, variable: Variable) -> Result<Cow<'static, str>> {
    ...
    match variable {
        ...
        Variable::CursorColumn => {
            ...
            let position = zemacs_core::coords_at_pos(text, cursor);
        ...
        Variable::FilePathAbsolute => {
            let path = match doc.path() {
                ...
                None => zemacs_stdx::env::current_working_dir(),
            }
            .to_string_lossy()
            .to_string();
            Ok(Cow::Owned(path))
