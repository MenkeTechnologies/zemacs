...
use zemacs_lsp::util::generate_transaction_from_edits;
use zemacs_lsp::{lsp, LanguageServerId, OffsetEncoding};
...
pub enum ApplyEditErrorKind {
    DocumentChanged,
    FileNotFound,
    InvalidUrl(zemacs_core::uri::UrlConversionError),
...
impl From<zemacs_core::uri::UrlConversionError> for ApplyEditErrorKind {
    fn from(err: zemacs_core::uri::UrlConversionError) -> Self {
...
impl Editor {
    ...
    pub fn handle_lsp_diagnostics(
        ...
        if let Some(doc) = doc {
            ...
            let doc = doc.id();
            zemacs_event::dispatch(DiagnosticsDidChange { editor: self, doc });
        }
    }
