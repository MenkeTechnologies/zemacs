...
// general utils:
pub fn dap_pos_to_pos(doc: &zemacs_core::Rope, line: usize, column: usize) -> Option<usize> {
...
pub fn jump_to_stack_frame(editor: &mut Editor, frame: &zemacs_dap::StackFrame) {
    let path = if let Some(zemacs_dap::Source {
        path: Some(ref path),
        ..
    }) = frame.source
    {
...
pub fn breakpoints_changed(
    ...
    let source_breakpoints = breakpoints
        .iter()
        .map(|breakpoint| zemacs_dap::SourceBreakpoint {
...
impl Editor {
    pub async fn handle_debugger_message(
        &mut self,
        id: DebugAdapterId,
        payload: zemacs_dap::Payload,
    ) -> bool {
        use zemacs_dap::{events, Event};
